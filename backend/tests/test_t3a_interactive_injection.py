...
def _post_expect_failclosed(svc, conv_id, cf, capture, reg, *, match: str) -> None:
    ...
    异常类型 = InteractiveToolAdmissionError（Codex R3-P2：typed FlaiError 令 API 层映射 503 配置错，
    旧裸 RuntimeError 会逃逸成 HTTP 500）。"""
    from backend.app.core.errors import InteractiveToolAdmissionError

    with pytest.raises(InteractiveToolAdmissionError, match="交互面工具安全门 fail-closed"):
...
def test_interactive_raw_files_or_isolation_tool_failclosed(tmp_path, monkeypatch) -> None:
    """★B1 R3-P1 tamper 锚（Codex R3 verbatim 扩跨字段）：工具即便 interactive_safe=true + internal
    + 无 shell，safety.save_raw_files=true 或 require_workspace_isolation=true 也必被拒——交互 wrapper
    调 ToolRegistry 不传 DB conn（留痕审计无从兑现=写盘工具无审计裸跑）、交互路径无 per-task 隔离
    workspace（隔离义务无从兑现）。tamper：从安全门旗标元组删 save_raw_files/require_workspace_
    isolation 任一支 → 对应工具被放行、workflow 被调 → 本测试红。"""
    for flag in ("save_raw_files", "require_workspace_isolation"):
        capture = _CaptureWorkflow()
        agent = _agent(tools=["rogue_writer_tool"])
        reg = _StubToolRegistry({
            "rogue_writer_tool": {
                "interactive_safe": True,
                "output_classification": "internal",
        ...
        subdir = tmp_path / flag
        subdir.mkdir()
        svc, cf = _build_service(
            subdir, monkeypatch, agent,
            tool_registry=reg, knowledge_service=object(), capture=capture,
        )
        conv_id = _open_conv(cf, agent["id"])
        _post_expect_failclosed(svc, conv_id, cf, capture, reg, match=flag)
