...
def test_zombie_thread_does_not_publish_after_reaper_failed(runtime_env, monkeypatch) -> None:
    """★A2 tamper 锚（Codex R1-P1-a 加强为原子认领）：模拟「reaper 墙钟超时把执行中任务置 failed，
    被放弃的僵尸执行线程仍跑到 workflow 成功尾部」。产物发布前经 repos.claim_publish_transition
    **原子认领**执行态→终态：任务已 failed（非执行态）→ 认领返 None → 放弃整个发布块（不注册产物、
    不写 sim_run_ref、不记 success 样本），任务保持 failed。tamper：把 runtime._execute 的
    `claim_publish_transition(...) is None → return failed` 换成无条件 set_task_outputs/record_sample
    （即回退非原子发布）→ 僵尸线程给已 failed 任务写 output_file_ids（脏发布）→ 本断言
    output_file_ids==[] / 无 success 样本变红。"""
...
# ── R1-P1-a（Codex R1）claim_publish_transition 原子认领终态：发布闸原子性 ─────────

def test_claim_publish_transition_running_to_waiting_review(runtime_env) -> None:
    """认领 running→waiting_review（单跳）→ 返回 status=waiting_review（人签态，非执行态）。"""
    conn_factory = runtime_env["conn_factory"]
    tid = "cpt_wr"
    _drive_to(conn_factory, tid, ("queued", "validating", "running"))
    conn = conn_factory()
    try:
        claimed = repos.claim_publish_transition(conn, tid, "waiting_review")
...
def test_claim_publish_transition_completed_passes_through_analyzing(runtime_env) -> None:
    """认领 running→completed：同事务内经 analyzing（docs/05 §2 running 不得跳过 analyzing）到
    completed，finished_at 落。tamper：把 claim completed 分支改成 running→completed 直翻（跳
    analyzing）→ assert_transition 炸 IllegalTransitionError（审计轨保护），本用例 raise。"""
    conn_factory = runtime_env["conn_factory"]
    tid = "cpt_done"
    _drive_to(conn_factory, tid, ("queued", "validating", "running"))
    conn = conn_factory()
    try:
        claimed = repos.claim_publish_transition(conn, tid, "completed")
        assert claimed is not None
...
def test_claim_publish_transition_returns_none_when_reaper_already_failed(runtime_env) -> None:
    """★R1-P1-a 原子性地基：任务已被 reaper 置 failed（非执行态）→ 认领返 None（放弃整个发布）。
    发布闸与产物写入同一意图：reaper 抢先即整体放弃，杜绝 un-fail + 脏发布。tamper：把 claim 的
    执行态白名单检查删掉（无条件翻 target）→ 已 failed 任务被认领成 waiting_review（un-fail）→
    本断言 is None 红。"""
    conn_factory = runtime_env["conn_factory"]
    tid = "cpt_failed"
    _drive_to(conn_factory, tid, ("queued", "validating", "running"))
    ...
    try:
        ...
        assert repos.claim_publish_transition(conn, tid, "waiting_review") is None  # 非执行态→None
        assert repos.get_task(conn, tid)["status"] == "failed"                       # 保持 failed
    finally:
        conn.close()


def test_claim_publish_transition_rejects_unknown_target(runtime_env) -> None:
    ...
    try:
        with pytest.raises(ValueError):
            repos.claim_publish_transition(conn, tid, "failed")
    finally:
        conn.close()
