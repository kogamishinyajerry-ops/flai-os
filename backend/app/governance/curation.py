...
# 进程内锁避免同进程重复工作；真正的跨进程序列化由外层
# ``BEGIN IMMEDIATE`` 提供，发布再用同目录 hard-link 的 no-clobber 语义兜底。
...
def _load_curation_context(
    *,
    ...
    if not isinstance(input_file_ids, list) or input_file_ids != []:
        raise CurationError(
            "源任务 input_file_ids 必须是精确空列表；V1 不自动把用户上传数据搬进 "
            "agent 包目录"
            "（需 Eval 维护者人工落文件后手写 case）"
        )
    if (
        task.get("agent_id") != sample.get("agent_id")
        or task.get("agent_version") != sample.get("agent_version")
    ):
        raise CurationError(
            f"样本 {sample_id} 与源任务的 agent/version provenance 不一致，fail-closed"
        )
    output_file_ids = task.get("output_file_ids")
    if (
        not isinstance(output_file_ids, list)
        or len(output_file_ids) > 512
        ...
        raise CurationError("源任务 output_file_ids 形态非法，拒绝 curation")
    output_files = repos.list_files_by_ids(conn, output_file_ids)
    if (
        [record.get("id") for record in output_files] != output_file_ids
        or any(
            record.get("kind") != "output"
            or record.get("task_id") != task.get("id")
            for record in output_files
        )
    ):
        raise CurationError("源任务 output file 关系无法完整证明，拒绝 curation")
...
def _scan_cases(
    ...
    # 与 Eval/Promotion 的事实源共用同一个分类器。只要已有任一 case 会被
    # runner 判为 broken，就必须在触盘前停止；不能在坏证据旁边继续叠加 draft。
    from .eval_runner import load_eval_cases

    try:
        _, _, broken = load_eval_cases(cases_dir.parent)
    except (OSError, UnicodeError) as exc:
        raise CurationPackageConflict(
            "eval case 目录无法完整读取；拒绝继续写 live package，须先人工修复"
        ) from exc
    if broken:
        first = broken[0]
        raise CurationPackageConflict(
            f"eval case {first.get('_file', 'unknown')} 已被共享 loader 判为 broken；"
    ...
    for path in sorted(cases_dir.glob("*.json")):
        ...
        if not isinstance(existing, dict):
            # 理论上已被共享 loader 捕获；保留二次读取期间外部漂移的 fail-closed 门。
            raise CurationPackageConflict(
                f"eval case {path.name} 顶层不是 JSON 对象；拒绝继续写 live package"
            )
        prov = existing.get("provenance")
        if isinstance(prov, dict) and "sample_id" in prov:
            provenance_sample_id = prov.get("sample_id")
            # Python equality makes True == 1 and 1.0 == 1.  Identity
            # provenance may never inherit that coercion: a non-int address is
            # malformed evidence, not an idempotent replay of an integer row.
            if type(provenance_sample_id) is not int:
                raise CurationPackageConflict(
                    f"eval case {path.name} 的 provenance.sample_id 非严格整数；"
                    "拒绝继续写 live package，须先人工修复"
                )
            if provenance_sample_id == sample_id:
...
def _assert_package_curation_writable(
    *,
    ...
    db_agent = repos.get_agent_version_and_maturity(conn, agent_id)
...
def _acknowledge_locked(
    *,
    ...
    try:
        ...
        except (repos.SampleAcknowledgementConflictError, ValueError) as exc:
