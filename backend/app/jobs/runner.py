...
class JobRunner:
    ...
    def __init__(
        ...
        *,
        ...
        # pending 终态化重试账本（Codex R3-P1 verbatim）：reap/线程启动失败/线程异常收口若遇**瞬时**
        # 存储故障（conn 打开/PRAGMA/BEGIN IMMEDIATE 锁超时），任务已离 queued——不留重试则永久搁浅
        # 执行态（心跳仍新鲜、无人再触达）。失败即把「重放同一收口动作」的 thunk 登记于此，run_once
        # 每轮先重放；成功/拒绝（人签/终态保护）即出账，仍瞬时失败则 thunk 内部重新登记。key=task_id
        # （同任务只留最新一笔，天然去重有界）。
        self._pending_fails: dict[str, Callable[[], None]] = {}
    ...
    def _retry_pending_fails(self) -> None:
        """重放登记在册的失败终态化动作（Codex R3-P1）：逐笔先出账再重放——重放内部若再遇瞬时
        故障会自行重新登记（见各收口方法），成功/拒绝则不再入账。thunk 自身兜底不上抛，此处
        try 仅防御 thunk 之外的意外，绝不炸 worker。"""
        if not self._pending_fails:
            return
        for task_id, retry in list(self._pending_fails.items()):
            self._pending_fails.pop(task_id, None)
            try:
                retry()
            except Exception:
                logger.exception(
                    "pending 终态化重放意外上抛（收口方法应自兜）：任务 %s，下轮再试", task_id
                )
                self._pending_fails.setdefault(task_id, retry)
    ...
    def run_once(self) -> bool:
        ...
        """
        # 先重放 pending 终态化（Codex R3-P1）：上轮 reap/收口遇瞬时存储故障的任务优先补终态，
        # 绝不让其搁浅执行态被新鲜心跳掩盖；空账本时零开销。
        ...
        def _execute_in_thread() -> None:
            try:
                ...
                box["done"] = True  # 完成 sentinel（Codex R2-P1）：join 后据此区分「正常完成」与
                #                     「线程异常退出无痕」，后者保守当失败终态化防搁浅。
            except IllegalTransitionError as exc:
                box["race"] = exc
            except BaseException as exc:  # noqa: BLE001 - ★含 SystemExit/KeyboardInterrupt（Codex R2-P1）：
                #  execute() 抛 BaseException 时线程会静默退出、box 无条目、is_alive()=false、两分支都
                #  不跑 → run_once 返 True 但任务永久搁浅执行态且心跳仍新鲜。捕获记 box["exc"]→
                #  _mark_failed_best_effort 终态化。daemon 执行线程吞 BaseException 不影响主循环 Ctrl-C。
                box["exc"] = exc
        ...
        # 线程已退出：逐分支回放。race→race warning；exc（含 BaseException）→失败终态化；done=
        # 正常完成（execute 内部已落终态）。★三者皆无=线程异常退出无痕（Codex R2-P1）→ 保守当失败
        # 终态化，绝不让任务搁浅执行态被新鲜心跳掩盖。
        if "race" in box:
            self._record_race_warning(task_id, agent_id, box["race"])
        elif "exc" in box:
            self._mark_failed_best_effort(task_id, box["exc"])
        elif "done" not in box:
            self._mark_failed_best_effort(
                task_id,
                RuntimeError("执行线程异常退出无完成/失败痕迹（保守置 failed 防搁浅，Codex R2-P1）"),
            )
        return True
    ...
    def _fail_thread_startup(self, task_id: str, exc: BaseException) -> None:
        ...
        # ★Codex R3-P1（verbatim，同 _reap_timed_out_task）：conn 打开/置 failed CAS 瞬时故障 →
        # 登记 pending 下轮重放（任务已认领到 validating，无重试即永久搁浅）。
        conn = None
        try:
            conn = self._conn_factory()
            ...
            logger.exception(
                "线程启动失败收口瞬时失败：任务 %s 置 failed 未达成，登记 pending 下轮重试"
                "（Codex R3-P1）",
                task_id,
            )
            if conn is not None:
                conn.close()
            self._pending_fails[task_id] = (
                lambda tid=task_id, e=exc: self._fail_thread_startup(tid, e)
            )
            return
        try:
            if failed_task is None:
    ...
    def _reap_timed_out_task(self, task_id: str, wall_s: float) -> None:
        ...
        # ★Codex R3-P1（verbatim）：conn 打开与置 failed 的 CAS 任一遇**瞬时**故障（SQLite 打开/
        # PRAGMA/BEGIN IMMEDIATE 锁超时）→ 登记 pending 重试后返回。旧行为：conn_factory 在 try 外
        # 抛→逃逸 run_forever 只记日志，而任务已离 queued、再无人触达=永久搁浅执行态（心跳仍新鲜
        # 掩盖）；内层 fail 异常也只记日志即丢。pending 重试令终态化 at-least-once（run_once 每轮
        # 重放直到成功/被人签终态保护拒绝）。
        conn = None
        try:
            conn = self._conn_factory()
            ...
            logger.exception(
                "M1 reaper 瞬时失败：任务 %s 置 failed 未达成，登记 pending 下轮重试"
                "（任务已离 queued，无重试即永久搁浅执行态，Codex R3-P1）",
                task_id,
            )
            if conn is not None:
                conn.close()
            self._pending_fails[task_id] = (
                lambda tid=task_id, w=wall_s: self._reap_timed_out_task(tid, w)
            )
            return
        try:
            if failed_task is None:
    ...
    def _mark_failed_best_effort(self, task_id: str, exc: BaseException) -> None:
        # exc 可为 BaseException（box["exc"] 含 SystemExit/KeyboardInterrupt，Codex R2-P1）——
        # 类型放宽与 _fail_thread_startup 一致；格式化对任何 BaseException 均安全。
        error_message = f"{exc.__class__.__name__}: {exc}"
        # ★Codex R3-P1（verbatim，同 _reap_timed_out_task）：conn 打开/置 failed CAS 瞬时故障 →
        # 登记 pending 下轮重放，绝不让已离 queued 的任务搁浅执行态。
        conn = None
        try:
            conn = self._conn_factory()
            failed_task = repos.fail_task_from_execution(conn, task_id, error_message)
            ...
            logger.exception(
                "run_once 兜底瞬时失败：任务 %s 置 failed 未达成，登记 pending 下轮重试"
                "（Codex R3-P1）；原始异常：%s",
            ...
            if conn is not None:
                conn.close()
            self._pending_fails[task_id] = (
                lambda tid=task_id, e=exc: self._mark_failed_best_effort(tid, e)
            )
            return
        try:
            if failed_task is None:
