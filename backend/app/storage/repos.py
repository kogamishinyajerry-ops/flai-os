...
def get_queue_depth(conn: sqlite3.Connection) -> dict[str, Any]:
    ...
    try:
        ...
        # ★Codex R3-P2（verbatim）：畸形检测与「取最老合法时间戳」**解耦**。旧 MIN(updated_at) 按
        # 字符串序取最小——合法 ISO（"2026-…"）常排在畸形文本（"not-a-timestamp"）之前，混杂队列里
        # 畸形行被 MIN 遮蔽、malformed 恒 false，违背「无法算龄=绝不假报健康」的 fail-closed 方针
        # （docstring 337 行）。改为取回该态**全部**时间戳、逐行 Python 解析：任一行畸形 → malformed
        # =True（就绪端点照方针 degrade）；龄期=全部**合法**行里最老（畸形行不再遮蔽也不再顶替）。
        # 开销 O(该态行数)/次：生产队列深度按单串行低 QPS 设计天然有界；benchmark 侧已限固定采样
        # 次数（见 measure_queue_throughput R3-P2），不放大为 O(N²)。
        def _oldest_and_malformed(rows: list[tuple[Any]]) -> tuple[float | None, bool]:
            oldest: float | None = None
            malformed = False
            for (raw_ts,) in rows:
                age, bad = _age_seconds(raw_ts)
                if bad:
                    malformed = True
                elif age is not None and (oldest is None or age > oldest):
                    oldest = age
            return oldest, malformed

        oldest_queued_age_s: float | None = None
        queued_malformed = False
        if queued > 0:
            # C1-P2-1：入队时刻 = updated_at（queued 任务的 updated_at 在 created→queued 时刷）。
            rows = conn.execute(
                "SELECT updated_at FROM tasks WHERE status = 'queued' AND origin = 'user'"
            ).fetchall()
            oldest_queued_age_s, queued_malformed = _oldest_and_malformed(rows)
        oldest_running_age_s: float | None = None
        running_malformed = False
        if running > 0:
            # A1：执行起始 = COALESCE(started_at, updated_at)（started_at 未设时回退 updated_at）。
            rows = conn.execute(
                f"SELECT COALESCE(started_at, updated_at) FROM tasks "
                f"WHERE status IN ({exec_ph}) AND origin = 'user'",
                _EXECUTION_STATES,
            ).fetchall()
            oldest_running_age_s, running_malformed = _oldest_and_malformed(rows)
        conn.execute("COMMIT")
    except Exception:
        conn.execute("ROLLBACK")
        raise
...
def finalize_publish(
    ...
    *,
    ...
    """物理产物已注册后，**原子**完成「attach 产物清单 + 记 success 样本 + 执行态→终态迁移」
    （Codex R2-P1；R1 claim_publish_transition 的升级——把 attach/sample 从翻态后的裸写并入
    同一 BEGIN IMMEDIATE，杜绝 R1 flip-first 的卡死终态）。

    R1 缺陷（本函数修的）：R1 先 claim_publish_transition 翻终态、再在事务外裸 set_task_outputs/
    record_sample。若翻态后的物理注册/裸写**抛异常**，任务已 completed/waiting_review 但产物缺失，
    reaper 只认执行态→够不着=卡死终态（completed 还可能放行下游=假绿）。R2 逮的正是此。

    正解（本函数 + 调用方协作）：调用方先 _register_outputs（物理文件拷贝，在**执行态**做——抛则
    任务留执行态、reaper/_mark_failed 可回收）；本函数只做**纯元数据**的 attach+sample+flip，全包进
    一个 BEGIN IMMEDIATE：
      1. 锁内 re-check status 仍在执行态白名单 {validating,running,parsing,analyzing}；
         否（reaper 抢先置 failed / 已并发推进）→ COMMIT 空事务返 None。
      2. set output_file_ids（attach 产物清单）。
      3. sample_spec 非 None → record_sample（success 样本，事务内、record_sample 不自 commit）。
      4. 按 target 翻终态：
         target='completed'：execution→analyzing→completed 多跳（docs/05 §2「running 不得跳过
         analyzing」，两跳同事务原子，reaper 的 fail 事务无法中途插入）；
         target='waiting_review'：execution→waiting_review 单跳（人签态）。
      5. COMMIT——attach+sample+flip 要么全落要么全不落，绝不出现「终态但产物缺失」。

    返回 None（reaper 抢先）→ 调用方放弃发布：_register_outputs 已建的产物文件行成为 orphan
    （files 表有行但**不在任何终态任务的 output_file_ids 内**）；全部下游消费者一律按 output_file_ids
    过滤（runtime 依赖解析显式「file_id not in output_file_ids → 拒消费」、curation/eval/api 同），
    orphan 行永不被引用=非脏（诚实边界：物理文件占盘直到清理回收，不影响正确性；用户裁决 accept
    此 tradeoff 换取「绝不卡死终态」）。返回任务用 `is None` 显式判定于调用方，绝不 truthiness
    （焊死红线）。target 白名单仅 {waiting_review, completed}，其余 raise（防误用）。"""
    if target_status not in ("waiting_review", "completed"):
        raise ValueError(
            f"finalize_publish 仅支持 waiting_review/completed，实得 {target_status!r}"
        )
