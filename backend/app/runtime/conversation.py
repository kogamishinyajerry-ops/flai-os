    ...
    def post_message(
        ...
        try:
            ...
            for _tool_id in (agent.get("tools") or []):
                ...
                # ★Codex R1-P1-3 跨字段拒：仅查 interactive_safe + output_classification 会放行一个
                # safety.allow_shell_command=true 的工具进交互面（可在 LLM 会话中途无逐轮人签触发 shell
                # 执行）。shell 能力与「交互面纯数据类」构造上互斥——**任一 allow_shell_command is True
                # 即拒**，不论 interactive_safe 如何标（tool.yaml 误标 interactive_safe=true 的纵深防御）。
                # 判定 is True 显式绝不 truthiness（缺省/None/False/"true" 字面均按非 shell 处理但此处只在
                # 显式 True 时拒——shell 声明本身缺省即 False=非 shell）。
                if (_tool_meta.get("safety") or {}).get("allow_shell_command") is True:
                    raise RuntimeError(
                        f"交互面工具安全门 fail-closed：agent {agent_id} 声明的工具 {_tool_id} "
                        "safety.allow_shell_command=true——shell 能力工具绝不得进交互面（即便误标 "
                        "interactive_safe=true）：LLM 会话中途可无人签触发 shell 执行（ADR-0028 §6）"
                    )
