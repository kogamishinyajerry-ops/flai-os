...
# Gate2-T1-M1 R3-P1（Codex R3 verbatim，worker 可见行为变更）：reap/线程启动/线程异常三收口路径
# 遇瞬时存储故障（conn 打开/锁超时）→ 登记 pending 重试账本、run_once 每轮先重放——旧 worker 该
# 场景任务永久搁浅执行态（心跳仍新鲜掩盖）。bump 代际逼 worker 重启到含 pending 重试的新代码。
WORKER_GENERATION = "collab-resolver+t2-eval-snapshot+b3-llm-timeout+m1-reaper+r1fix-lifecycle-atomic+r2fix-toollifecycle-finalize-baseexc+r3fix-pendingreap"
