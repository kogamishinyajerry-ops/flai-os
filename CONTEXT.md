...
**会议闭环**:
会议负责人确认正式纪要，且每项责任事项均已明确负责人和期限；责任事项的执行与验收属于后续独立生命周期。
...
**责任事项**:
从正式会议记录产生的可验收承诺，必须具有明确产出、唯一负责人、截止时间、验收标准和验收人；会议负责人签发正式会议记录后立即生效，任一必需信息缺失时仍是待处理例外。
...
**控制内核**:
FLAi-OS 中唯一拥有身份、策略、任务图、队列、权威状态、审计证据与交付决定的控制边界；任何外部 Agent 框架都不能成为第二事实源。
...
**受控验收试点**:
由 5–8 名具名技术验收人员在 canonical 或 synthetic 数据上验证自治体验、安全控制和证据链的首轮试用；它证明平台机制，不证明真实业务价值或生产就绪。
...
**工程智能体工作台**:
工程师把自然语言目标交给自治会话、持续查看真实执行状态并检查产物与证据的默认产品界面；它是 FLAi-OS 首发主产品。
_Avoid_: 战略驾驶舱、项目管理后台、智能化指挥中心

**治理与运行中心**:
平台管理员、安全人员和 Agent Owner 管理能力生命周期、策略、执行资源、异常与审计证据的角色受限界面；它只投影控制内核事实，不形成第二控制面。
_Avoid_: 普通用户首页、第二管理平台、全员驾驶舱

**智能化指挥中心**:
面向领导和 AI 负责人的后期只读管理视图，从已验证的任务、评测、交付和风险证据派生组织层信息；它不拥有任务、授权或指标的独立事实源。
_Avoid_: FLAi-OS 产品总名、工程师工作台、手填战略看板
...
**能力发布包**:
一次可被评测和发布的完整能力版本，绑定 Agent、Prompt、Workflow、Schema、实际模型、Tool、Sandbox 与权限策略、ReleaseKnowledgeBinding 及评测包；任一关键成分漂移都会形成不同被评对象。任务时 TaskKnowledgeSnapshot 不反向定义该包身份。
_Avoid_: Agent 版本号、模型名称、代码提交号

**发布知识绑定（ReleaseKnowledgeBinding）**:
能力发布包在运行前冻结的知识合同，规定允许的权威范围、版本选择策略、目录根摘要和检索/锚点规则；不包含具体 actor、任务时刻或实际命中。
