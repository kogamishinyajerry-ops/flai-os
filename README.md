...
19. **协作会话生命周期与 GC 未完备**（M8/ADR-0016）：单 Agent 计划确认沿用 M6 归档
    （conclude）；多 Agent 会话保持 active 作协作锚点，**显式「结束协作」按钮与会话/
    孤儿附件 GC 留 V0.2**（同 #5/#15 同源，无主会话与残留文件的回收统一在 V0.2 做）。
    会话视图的成员任务状态靠手动「刷新」拉取（无轮询，同 #6）。
