...
11. **导引推荐的确定性校验只保证 shape 合法，不保证是模型真实意图**（M7 敌意审
    ...
    仍拒；③ADR-0031/0032 的 safe_auto 只会物化通过当前轮严格来源对账、版本锁定、
    角色门与无副作用门的计划；legacy 扁平计划仍只允许无附件单 Agent，
    `guide_dag.v1` 则额外要求逐节点来源绑定、唯一叶节点人审，任一证据不足即整图零任务；
    ④正式工程 review 仍只能由认证真人签发。工程师应把推荐卡片当建议核对，尤其
    top_event/system_description 等自由文本字段。
12. **非 safe-auto 会话附件仍有 file_id 归属欠账**（M7 敌意审 P2）：
    ADR-0032 已在 safe_auto 调模型前强制当前上传 username、`kind=input`、
    未归属任务、`classification=internal` 与 sha256，并拒绝历史轮附件；但
    `plan_only`/legacy 会话仍只校验 file_id 存在，知道其他文件 id 时仍可能把内容
    渲染进模型上下文。因此外部模型/EAR/商密场景仍必须限制在 safe_auto 窄通路，
    全会话文件 capability 矩阵仍是后续项。
...
18. **安全协作计划可自动执行，最终工程签发仍只属于人**（ADR-0031/0032）：
    legacy `agents[]` 仍只物化完整、无附件的单 Agent 计划；版本化 `guide_dag.v1`
    可原子创建 1–5 个拓扑有序节点，根节点 queued、下游 created，由现有 SQLite
    resolver 推进。参数必须来自当前轮严格 `inputs_by_agent`，当前轮 internal
    附件必须全集且仅绑定一个节点；任一节点版本、角色、来源或无副作用策略不满足就整图零任务。
    非叶节点只能是无模型确定性 Agent，唯一叶节点必须停在人审。浏览器以
    `sessionStorage` outbox 先写后读、复用稳定 request ID，再用 canonical GET + SQLite
    receipt 确认，因而支持同标签页硬刷新；不承诺关闭标签页/浏览器重启，也不宣称多 worker
    模型 single-flight。历史 `plan_only` 会话仍保留逐个召集入口；自动化的是机械创建与
    参数/来源搬运，不是工程签发。
