...
数据行数缺省上限 **1000**（`max_rows` 参数可放宽至 5000）：超限**整表诚实
拒绝**（status=failed「行数超上限，请分表」），绝不静默截断冒充全量结果。

## 已知限制

- 只读首个（或指定）sheet；多 sheet 工作簿的其余 sheet 忽略。
- 数值一律按 float 解析；布尔单元格（TRUE/FALSE）不是合法数值，判行级错误
  （防 float(True)==1.0 静默吞入）；不支持公式单元格以外的富文本/合并单元格语义。
