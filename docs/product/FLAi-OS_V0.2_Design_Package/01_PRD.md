...
现有 FLAi-OS 已证明轻内核、Agent/Tool Package、任务事件、模型网关、协作会话和评测治理等基础结构可以成立，但这些结构尚未自动形成一个中国工程用户愿意长期使用的企业 Agent 产品。当前需要同时解决四类问题：
...
| 导引/协作工作台/safe_auto | 已有会话、任务召集和受限自动创建 | `IMPLEMENTED-PARTIAL` | 不能把当前“单 Agent、无附件、无工具/副作用”的 safe_auto 称为 ADR-0034 目标会话 |
...
| ID | 需求 | 验收要点 | 状态 |
|---|---|---|---|
| WB-01 | 默认首页提供一个目标导向 Composer，可附加工作流允许的文件 | 发起三条黄金工作流不要求先填写 Agent 设计卡、权限矩阵或内部 JSON | `ACCEPTED-NOT-IMPLEMENTED` |
...
| MEET-04 | 会议负责人签发精确版本 | Phase 0A 仅产生隔离 `SYNTHETIC/TEST` receipt，不创建正式记录或激活真实责任；真实组织签发须待 R6/获批 Phase 0B | `ACCEPTED-NOT-IMPLEMENTED` |
...
| ID | 需求 | 验收要点 | 状态 |
|---|---|---|---|
| BENCH-01 | 评测对象是冻结能力发布包 | Agent、prompt/workflow/schema、实际模型、Tool/Adapter、Sandbox、策略、ReleaseKnowledgeBinding、用例/rubric 均有版本或摘要 | `ACCEPTED-NOT-IMPLEMENTED` |
...
- 数据清单只包含批准的 canonical/synthetic 材料，真实敏感数据测试必须失败；
...
以下均为 `DECLARED-NOT-VERIFIED`，需要在架构评审/MVP 定义中得到具名裁决：

- Phase 0A 的具体 5–8 人名单、签发资格和职责委派方式；
- DOCX 支持版本、保真判据、文件大小/复杂度预算和恶意文档策略；
- OpenFOAM 首个支持版本、求解器集合、字段解析规则和算例包预算；
- 会议输入格式、来源锚点粒度和正式会议负责人的身份契约；
- Sandbox 技术实现、资源预算、网络策略和强杀证明方法；
- 权威知识类别、生命周期、精确锚点与冲突算法的最终合同；
- capability-release manifest、四轨结果 envelope 和三套 rubric；
- 共建地图节点合同、需求事件合同、通知/回告 Adapter 和指标定义注册表。

## 14. 追踪关系

| PRD 范围 | 决策来源 |
|---|---|
| 唯一控制内核、可替换执行后端 | ADR-0033 |
| 不打断会话、末端 Delivery Bundle | ADR-0034 |
| Phase 0A/0B | ADR-0035 |
| 工作台、治理中心、共建地图、指挥中心顺序 | ADR-0036、ADR-0043 |
| 三条黄金工作流及首个 tracer bullet | ADR-0037～0039 |
| 权威知识 | ADR-0041 |
| FLAi Bench | ADR-0042 |
| 需求共创、路线图签发与职责分离 | ADR-0044～0044 |
