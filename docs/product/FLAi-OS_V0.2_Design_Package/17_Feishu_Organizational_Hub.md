...
上述路径只描述供体快照 `9220cc3` 中的评审包合同。本次
`flai-v02-authority-docs-port-codex-001@1` 明确排除全部 JSON Schema、评审执行包与
`CODEX_HANDOFF_PROMPT.md`，因此当前 main 候选不能实例化、验证或关闭该 F0 包，也不能把
下文的字段约束报告为已落地接口。需要这些机器合同时必须另冻 Schema/评审包移植工作项。
...
未来另冻并移植机器合同时，`normative_files[]` 至少完整覆盖 `CONTEXT.md`、
ADR-0047～0064、本设计包 README、00～19、届时另行复核并获准的
`CODEX_HANDOFF_PROMPT.md` 与对应 review-package Schema。由于当前 V1 Schema 在
ADR-0063/0064 与设计 18/19 形成前冻结，后续移植不得原样复用其 normative set；必须升级
合同版本、重算 frozen tree/manifest，并以完整集合运行 invalid-first 验证。
`normative_role` 只能是：
