...
> 首要依据：[ADR-0041](../../adr/ADR-0041-authoritative-knowledge-foundation.md)。
> 相关依据：[ADR-0015](../../adr/ADR-0015-knowledge-kernel-service.md)、[ADR-0017](../../adr/ADR-0017-knowledge-qa-agent.md)、[ADR-0021](../../adr/ADR-0021-m11-b2-data-classification.md)、[知识与记忆标准 V0.1](../../06_Knowledge_Memory_Standard.md) 与 [CONTEXT.md](../../../CONTEXT.md)。
...
| `IMPLEMENTED-PARTIAL` | 已有可复用骨架，但语义或覆盖范围尚不足以满足 ADR-0041 |
...
平台只提供一个逻辑权威目录和一个受控查询 Interface；内容字节可以留在受控文件目录、文档管理系统、Obsidian Vault 或其他内网系统中。物理位置改变不能改变权威等级，接入新的来源也不能产生第二个事实源。
...
Obsidian 可以是优秀的编辑、链接和策展界面，但它没有发布权。把 Markdown 文件放进 Vault、添加标签或建立双向链接，只能改变工作材料；只有受控发布动作才能产生权威知识项版本。
...
```mermaid
flowchart LR
    A["受控文件目录 / DMS / Obsidian / 内网系统"] --> B["Source Adapter"]
    B --> C["权威目录与版本模块"]
...
- **Replaceability**：更换 Obsidian、文档系统、向量库或执行后端，不改变权威目录及历史任务事实。
- **Auditability**：每次回答依赖的版本、策略和锚点都绑定任务，源系统后来变化也不能改写历史。

## 8. 发布知识绑定与任务时权威知识快照

能力发布包先冻结 `ReleaseKnowledgeBinding`：允许的 authority/scope、必需或禁止集合、版本选择策略、目录根摘要、检索/锚点策略和外部活态模式。它不包含 actor、任务时间或实际命中，因此可以在运行前参与 release digest。
...
2. 评测运行与业务运行可以绑定同一知识版本集合，避免“同一个 Agent、不同知识底座”却被误认为同一被评对象。
...
- Obsidian 等策展界面可组织草稿、补元数据、建议分类与锚点；
- 系统集中展示缺失的发布主体、批准证据、适用范围、时态、替代关系和密级；
- 具名授权人员对精确版本摘要执行发布；受信源系统只能提交包含上游签发身份、版本、内容 digest 与验证结果的 `source_system_attestation`；
...
7. 以 Adapter 增加 Obsidian/DMS/向量检索，不改变发布事实和历史任务事实的所有权。
...
1. 未发布的 Obsidian 文件、普通上传、会议笔记和 AI 产物即使被索引命中，也不能支撑正式结论。
...
13. Obsidian/file_dir/DMS 等不同 Source Adapter 通过同一发布与快照 Interface；任何 Adapter 绕过目录直供 Agent 的路径有测试拒绝。
...
ADR-0041 已确定方向，但编码前仍需形成可评审实施规格：
