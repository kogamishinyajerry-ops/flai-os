...
>
> 决策依据：[ADR-0043](../../adr/ADR-0043-co-building-map-and-evidence-derived-metrics.md)、[ADR-0044](../../adr/ADR-0044-demand-co-creation-loop.md)、[ADR-0045](../../adr/ADR-0045-demand-decision-rights-and-roadmap-signoff.md)
...
默认入口仍是工程智能体工作台。普通用户首页只展示共建地图摘要：当前路线图版本、本周真实解锁能力、下一目标、近期里程碑和“提交需求”入口；完整地图是一个独立只读页面。治理与运行职责可以在相同节点上查看更多评测、风险、授权和审计证据，但仍通过对应治理模块完成动作，不能在投影层写回事实源。
...
| 能力发布包 | 冻结的版本、适用范围、限制、运行依赖和退役关系 | Capability Release Package | 当前分支、代码合并、未冻结配置 |
...
通知由 `NotificationOutbox` Interface 异步投递，使用事件 id 作为幂等键。通知失败不回滚权威决策，但必须可重试并显示“待回告”；敏感信息只能发送脱敏摘要和授权内深链。本阶段不指定邮件、飞书、Teams 或其他连接器。
...
1. 关联明确的 Capability Release Package 及适用范围；
...
| Governance Policy | 作用域能力、适用领域/安全门、提交前复核 | `authorize`, `required_reviews`, `recheck_at_commit` |
...
通知连接器、内网替代 Issue tracker、更丰富的组织价值模型和 Intelligence Command Center 只在基础证据链稳定后接入。它们通过既有 Interface 扩展，不获得绕过签发、分类、Benchmark 或审计的捷径。
