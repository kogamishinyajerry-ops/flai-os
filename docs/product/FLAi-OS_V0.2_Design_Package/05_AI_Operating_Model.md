...
>
> 本文是 V0.2 设计包的运营读模型，不覆盖 [FLAi-OS 系统宪法](../../00_FLAi-OS_Constitution.md)、现行标准或已接受 ADR。术语以 [CONTEXT.md](../../../CONTEXT.md) 为准；需求与发布决策分别受 [ADR-0044](../../adr/ADR-0044-demand-co-creation-loop.md)、[ADR-0045](../../adr/ADR-0045-demand-decision-rights-and-roadmap-signoff.md) 和 [ADR-0042](../../adr/ADR-0042-flai-bench-evaluation-foundation.md) 约束。
...
这些 Module 通过稳定引用协作，不复制对方的权威状态。需求池是发现事实源，共建地图是路线图承诺事实源，GitHub Issue 或批准的内网追踪器是工程交付事实源，能力发布包是被评测与发布对象，FLAi Bench 是准入证据事实源。展示层只做投影。
...
每一步只消费上一步的稳定引用和证据，不通过聊天总结或会议口头结论隐式推进。失败、未知、跳过、证据不可解析均保留原状态并生成具名处理事件；系统不得自动寻找“更容易通过”的模型、策略或数据集。
...
- 人群：5–8 名具名技术验收人员；
- 环境：macOS 首发环境，canonical/synthetic 数据；
...
| 节奏 | 输入 | 输出 | 禁止 |
|---|---|---|---|
| 持续 | 需求、任务失败、用户反馈、安全事件 | 不可变信号和追加式事件 | 在用户任务中强插长表单 |
