...
| 任务、会话与工作台骨架 | 任务页、导引会话、协作工作台、任务分组、受限 safe_auto 已存在 | `IMPLEMENTED-PARTIAL` |
...
- `ACCEPTED-NOT-IMPLEMENTED` ADR-0034 的不中断自治会话、SessionExecutionGrant、Execution Broker、可强杀 Sandbox 与 Delivery Bundle 尚未作为完整链验收；
...
- 控制内核 Module 图与所有权表：Identity/Policy、Canonical TaskGraph、Session Execution、Sandbox、Artifact、Audit、Delivery Bundle；
...
|---|---|
| 人群与设备 | 5–8 名具名技术验收人员、macOS 版本、硬件最低配置、签发资格 |
| 数据 | canonical/synthetic 清单、分类、来源、摘要、允许的输入/输出位置 |
...
| Bench | 三套样本包、rubric、必测门、真实/synthetic 标记、可比性规则 |
...
- 所有开放问题都有具名裁决或成为明确 Stop-if，不留“开发时再看”；
- capability-release manifest 和 Phase 0A release manifest 可以由工具校验；
- owner 签发 MVP 版本；此时才能进入原型设计。
...
2. 用户无需理解 Agent id、内部 schema、TaskGraph 或权限枚举即可发起任务；
...
| 工作包 | 主要结果 | 依赖 | 完成门 |
|---|---|---|---|
| K1 Control Kernel ownership | Canonical TaskGraph、SessionExecutionGrant、策略解析、任务/状态唯一写入 | R0/R1 | 试图建立双控制面或让后端 Adapter 写权威终态的负例被拒绝 |
| K2 Sandbox & Execution Broker | 进程/文件/资源/时间/网络隔离，取消与强杀，receipt | K1 | 逃逸、超预算、外联、残余进程负例咬合 |
...
三条工作流可以在共同地基稳定后并行开发；ADR-0037 没有授权一个跨工作流的固定开发优先级。资源不足时，路线图负责人必须以独立版本记录选择和理由，不能让实现顺序反写成产品重要性。
...
4. 三个候选均只使用被批准的 canonical/synthetic 数据；
...
- `01_PRD.md` Phase 0A Gate A～E 全部通过；
...
候选必须同时满足：真实需求来源明确、数据授权可闭合、工具或材料可冻结/追溯、领域 reviewer 可参与、错误后果可控制、回退可执行。性能盘可作为成熟专业候选，但其既有 Mock/技术样板不能替代真实 Tool Adapter 与业务验收。
...
治理中心随 K1～K8 逐步形成角色化视图，但普通用户不需要先进入后台才能工作。任何治理操作都通过同一 policy Seam 和追加审计，不在 UI 内复制权限判断。
...
当前文档状态为 `ACCEPTED-NOT-IMPLEMENTED` 的工作路线读模型，不是正式排期承诺。正式共建地图版本必须记录：
...
| 路线范围 | ADR |
|---|---|
| 控制内核、Execution Broker 与三类执行 Port | ADR-0033 |
| 连续会话与末端交付 | ADR-0034 |
| Phase 0A/0B | ADR-0035 |
| 工作台先行、治理角色化、指挥中心后置 | ADR-0036 |
| 黄金工作流与薄切片 | ADR-0037～0039 |
| 权威知识 | ADR-0041 |
| FLAi Bench | ADR-0042 |
| 共建地图与指标 | ADR-0043 |
| 需求闭环与路线图签发 | ADR-0044～0044 |
