...
> 受控移植状态：`SOURCE_DOCUMENT_PORT_CANDIDATE_NOT_INTERNAL_RELEASE`
>
> 当前集成基线：`origin/main@ec43768ff1ad2cb6bc3e571486bfda2d04c35780`；
> 文档供体快照：`9220cc3eef138ca3d7a9503a0dcb66fb91d15511`。
>
> 本轮只移植 `CONTEXT.md`、ADR-0047～0064 与本目录 Markdown。运行时代码、
> JSON Schema、评审执行包、Agent dispatch/接手提示词均未移植；缺少这些文件不得解释为
> 对相应实现、评审或执行路径的授权。冲突时以当前 main 上 owner 已接受的接手 SSOT、
> 唯一开发基线裁决与“受限选项 2”方向裁决为准。
>
> Stage B 状态：`FROZEN-FOR-STAGE-C` / `ACCEPTED-NOT-IMPLEMENTED`（2026-07-23）
>
> Stage C 状态：`HISTORIC-SOURCE-CANDIDATE-ACCEPTANCE-RECORDED` / `DONOR-ONLY` /
> `PROTOTYPE-ONLY`（源码候选 `0d095df`，2026-07-24；未集成、未正式签发，
> 不授权从 donor 开始新工作）
>
...
owner 历史上曾接受 `0d095df` 对应的外网源码候选；唯一开发基线裁决随后将 Stage C
所在线明确降为 `DONOR_ONLY / EVIDENCE_ONLY`，并要求所有新工作从刷新后的 main 创建。
因此该历史接受只保留候选内容与取证价值，不再提供开发起点或执行授权；任何 Stage C
移植/重建都必须另冻 main-based 工作项。源码未进入 main，Stage C 未获组织正式签发，
Stage D 也没有打开。

### 1.1 决策接受与正式签发不是一回事

ADR-0049 至 ADR-0064 中的“已接受”，只记录委托人在本次设计会话中对产品方向的明确确认；ADR-0047/0048 负责谱系与术语身份澄清。它们都不是经组织身份系统认证的电子签名，也不自动等同于立项、预算、密级、试点、发布或上线批准。当前文档移植候选尚未形成可正式签发的 release digest，正式治理流程还必须为每项需签发决策补齐具名 `actor_id`、职责/作用域、时间、精确 ADR/制品 digest、决定和不可抵赖证据。缺少这些字段时，实施者只能把它当作已确认的设计输入，不能当作正式准入凭据。
...
这条记录确认的是 Stage C 的收敛方向，不是完整原型的组织正式验收，也不是
`STAGE-C-ACCEPTED-FOR-STAGE-D`。`0d095df` 的历史源码候选接受现只作为 donor evidence，
没有形成组织身份系统的 Stage C gate receipt，也不允许从 donor 分支启动新工作；
Stage D 因而继续关闭。
...
供体历史证据：曾在隔离工作树 `codex/flai-v02-foundation`、基线
`7523edf2cb94958082c68ef3b1aeea0b66b83905` 上运行窄范围回归；后端
`test_m11_auth.py`、`test_runtime.py`、`test_job_runner.py`、`test_eval_snapshot.py`、
`test_knowledge_service.py`、`test_file_integrity.py` 共 `90 passed, 1 warning`，前端纯函数核
`node --test` 共 `29 passed`（2026-07-23，macOS）。这些数值只属于供体时点，不能替代本次
移植验证，也没有证明 V0.2 目标实现；由于设计文件仍未形成正式 release digest，且没有真实
Sandbox、真实三条 tracer 或生产验收，本包仍把相关基线统一写为
`IMPLEMENTED-PARTIAL`，不得用这些结果提升试点或生产状态。
...
| `docs/agents/codex-kimi-dual-thread.md`（供体证据，本轮未移植） | 双 work item、独立 scope/worktree、witness、handoff 与人类集成纪律 |
| `docs/reviews/production-snapshot-assembler-read-v1/`（供体证据，本轮未移植） | 绑定冻结 SHA 的七域评审计划、Decision Core/Seal 模板与 fail-closed 机械检查 |
| `CODEX_HANDOFF_PROMPT.md`（执行提示词，本轮明确排除） | 另冻工作项后才可评审或移植，当前不存在可执行接手授权 |
...
10. 若工作涉及 Stage C 真实运行观察，先评审 [16_Production_Snapshot_Assembler_Read_Contract.md](16_Production_Snapshot_Assembler_Read_Contract.md)；供体中的七域具名评审控制包本轮未移植，在其另冻、移植、复核且取得实施授权前不得实现 Production Snapshot Assembler；
11. 外网研发协作工作再读 `17_Feishu_Organizational_Hub`；它不再定义内网产品入口；
12. `CODEX_HANDOFF_PROMPT.md` 属执行提示词，本轮没有移植；需要时另冻工作项，不能从本设计包直接进入编码。
