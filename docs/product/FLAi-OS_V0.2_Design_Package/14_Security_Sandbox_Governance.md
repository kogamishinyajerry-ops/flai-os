...
> 现状证据基线来自 [OpenClaw / WorkBuddy / FLAi-OS 安全治理研究](../../research/2026-07-23-openclaw-workbuddy-security-governance-study.md)、
> [生产就绪纲领](../../PRODUCTION-READINESS-PROGRAM.md) 和已接受 ADR。
...
最小对象：request/session/TaskGraph、task、conversation、file/artifact、feedback、Grant/lease/ExecutionRun、agent/tool/connector、eval/Bench、sample、typed promotion/qualification、CapabilityRelease/DeploymentBinding、knowledge、delivery/attempt/receipt、demand/roadmap、audit view、project membership。最小动作：read、list、search、export、aggregate、write、execute、cancel、review、approve、publish、revoke、audit。
...
临时目录、Python 线程 timeout、进程组或清洁环境只能称 containment。Sandbox Backend 不可用、健康未知或策略无法表达时，Execution Broker 必须拒绝，不得降级到宿主执行。
...
- **状态**：`ACCEPTED-NOT-IMPLEMENTED`。
- 配置只保存 `secret://provider/scope/name@version`，业务代码不保存长期明文。
...
- macOS 可评估 Keychain/受控文件 Provider，但具体实现必须另行威胁建模与验收。
...
- 安全事件处置应支持撤销会话/Grant/Secret/包版本、停止 lane、隔离制品、验链、导出证据和恢复；每项动作仍受身份与审计约束。
...
3. Execution Broker + 一个低副作用 Tool + 最小真实 Mac Sandbox/kill；
...
| 5–8 人 Phase 0A 的峰值任务、Tool/模型并发 | lane/budget 和容量基准 |
| 适用的等保、密码应用或其他组织控制 | 合规范围；本设计包不作认证结论 |
| ADR-0033～0045 的正式决策签发载体与签发人 | 当前会话确认只证明设计方向接受；进入实施/试点前须绑定身份、职责、时间和精确 digest |
...
Phase 0A 通过只证明限定机制，不证明生产就绪或真实业务价值；进入 Phase 0B 还需要至少一条真实工具、真实数据工作流的安全和准确性验收。见 [ADR-0035](../../adr/ADR-0035-two-stage-controlled-and-business-pilots.md)。
...
- [ADR-0030：subprocess 与出站边界](../../adr/ADR-0030-ax-web-extract-subprocess-and-egress-boundary.md)
- [ADR-0031：认证 safe-auto 边界](../../adr/ADR-0031-authenticated-safe-auto-dispatch.md)
...
- [ADR-0045：需求治理职责分离](../../adr/ADR-0045-demand-decision-rights-and-roadmap-signoff.md)
