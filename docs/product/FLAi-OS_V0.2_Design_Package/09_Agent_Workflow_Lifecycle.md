...
尚未取得资格的 frozen candidate 只能由具备职责的 Eval maintainer 签发 `EvaluationAdmission` 后运行实验室 Bench。该事实绑定 evaluator/service actor 与 scope、精确 release、approved synthetic fixture/pack/rubric/Gate Policy/environment digest、允许动作、`external_effects=none`、资源/Token 预算、TTL 与 ExecutionEpochSnapshot，并可 `active|suspended|revoked|expired`。`origin=eval` 只是 Kernel 验证该事实后派生的审计标签，任何客户端伪造标签、普通用户、真实数据、错 pack/release、预算/epoch 漂移或外部效果请求都拒绝。它不产生资格或部署暴露。

### 3.6 Kernel 的 effective_callability

Kernel 只在共同条件与一条精确调用路径全部明确为 true 时接纳任务。共同条件是包为 `frozen`、Agent 非 `disabled`、对象授权与依赖健康且 Session Grant 可被有效派生。实验室评测路径还必须有与 actor/scope/release/approved synthetic fixture/pack/预算/TTL/epoch 精确匹配的 active EvaluationAdmission；用户/业务路径还必须有目标 deployment class 的唯一 current eligible QualificationDecision、匹配且 active 的 DeploymentBinding，以及主体/项目/数据/动作/时间范围匹配。两条路径不得互相降级或回退。接纳后依次完成 Queue admission、claim lease 与 Broker prepare；只有绑定当前 lease generation 的具体 StepProposal 再次通过授权，才签发短时 ExecutionTicket。`suspended/revoked/expired/unknown` 任一命中都硬否决。L0–L3 只描述成熟度，不授予调用权限。
...
- Gateway 实际解析模型 endpoint/route/trust bundle、参数档与 tokenizer id/version/package digest；
- Tool/Adapter、Sandbox 镜像和策略版本；
- 完整 canonical ReleaseKnowledgeBinding 及 `binding_digest`（allowed authority/scopes、required/prohibited sets、版本选择、目录根、检索/锚点策略和 external-live declarations）；运行时 TaskKnowledgeSnapshot 由每次任务按 Binding 解析，不参与发布包 digest；
- 评测包、rubric、Gate Policy、预算档和环境等级；
...
安全泄露、越权、无法强杀、审计链断裂、关键依据错误、假绿或大面积不可恢复失败，应通过 typed Deployment Interface 暂停/撤销暴露并停止新 admission。Control Kernel 的同一事务中，由 DeploymentBinding 的唯一 owner Module 写入绑定转换、Authorization Module 唯一 CAS 提升对应 authorization partition epoch，并共同写 `revocation_id`、受影响 lease selector、CancellationToken 与 invalidation outbox；普通协作/工作负载 Identity 或 Supply-chain 撤权则分别由其唯一 owner 提升 credential/trust-policy epoch。独立 Safety Identity / PKI / HSM 的 hardware admission/key epoch 由其自身 owner 管理，Kernel 只消费可验证结果。所有绑定旧 `ExecutionEpochSnapshot` 的 active lease 在冻结 SLA 内进入 Broker 强杀、步骤凭据吊销与既有连接失效；缺 termination/credential/connection witness 时状态只能是 `revocation_incomplete/needs_reconciliation`，不能记“已取消”。历史证据保持可读。
...
- Phase 0A 使用 approved synthetic 数据（包括 `fixture_class=canonical` 的正常样本）通过后直接宣传真实业务有效；
