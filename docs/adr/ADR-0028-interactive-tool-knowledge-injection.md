...
6. **副作用工具边界 —— 代码 fail-closed 已强制（T3-fix，Codex C3-P1-2/P1-3 命中即审）**：交互工具
   ...
   **本轮 T3-fix 已把该边界从文档声明升级为代码 fail-closed 强制**：
   ...
   - **R3 加固（Codex R3，逐字落地 Suggested fix 按 verbatim 例外不再走一轮）**：①**逐条引用覆盖**——
     `_grounding_status` 从「≥1 有效键且无编造」收紧为**每个实质结论单元**（切句启发式，去键后 ≥12 字）
     须自带有效键或显式「语料未覆盖」声明，否则整篇 amber 未核（单键混合答复不再坐享背书出处表，R3-P1）；
     ②**assistant 历史轮同过中和**——用户可诱导模型在答复里生成 `<<KNOWLEDGE>>` 样式文本、第二轮 replay
     冒充平台 fence，prior_turns 的 assistant 轮同过 `_neutralize_sentinels`（本 Agent 合法答复不含
     `<<`/`>>`，零损，R3-P1）；③**跨字段拒扩容**——`safety.save_raw_files is True`（交互 wrapper 不传
     DB conn，留痕审计无从兑现）与 `require_workspace_isolation is True`（交互路径无 per-task 隔离
     workspace）同 shell 一样构造性互斥即拒（R3-P1）；④**安全门 typed 异常**——门内 RuntimeError 全部
     改抛 `InteractiveToolAdmissionError(FlaiError)`，API 层按永久性资源配置错映射 503（旧裸 RuntimeError
     逃逸成 HTTP 500，R3-P2）。
