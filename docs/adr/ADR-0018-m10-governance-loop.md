...
- 只接受 `completed + validation_status=success + classification=internal` 且源任务
  不含输入文件的样本；既有 `accepted_by_engineer=false` 是不可覆盖的人工拒绝，
...
- live package 中任何现存 `*.json` 无法读取或解析时，sample acknowledge 与旧
  `/eval-cases` 均在触盘前 409，保留 broken 证据并等待人工修复；不得跳过坏
  case 后继续叠加第二份 sample provenance。
