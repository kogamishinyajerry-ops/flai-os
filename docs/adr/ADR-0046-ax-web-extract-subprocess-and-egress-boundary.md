# ADR-0046 · ax 静态 Web 工具的 subprocess、出站与证据边界

- 状态：**已采纳（仅 L0 离线契约批）**；生产联网激活仍 **BLOCKED**
- 日期：2026-07-21
- 关联：ADR-0008（工具超时诚实）· ADR-0021（数据分级）·
  ADR-0022（受控 subprocess 先例）· ADR-0024/0025（工具污点与不可变分级）·
  ADR-0045（运行时角色轴仍为独立 owner 决策）· docs/03 Tool Package 标准
...
顺序焊死如下；本 ADR 不授权其中任何一项实施：

1. 先完成运行时角色轴，并在创建、执行、证据读取/下载与签发入口共享同一条
   default-deny policy seam；未知角色、停用用户、越权访问和同任务自审必须有负例；
2. 再完成可执行 egress policy：连接前 DNS/目标 IP 校验、loopback/link-local/private/
   metadata 拒绝、每跳 redirect 复核、域名/端口 allowlist、出口代理或防火墙及可验证
   policy identity，不能只写文档或传入一个布尔开关；
3. 只有前两项均有独立验证证据后，才允许另提并开发 ax L1 生产 adapter；L1 adapter
   本身不是解锁信号，必须默认 disabled，并在运行时精确绑定角色判定与 egress policy
   witness，缺任一项都 fail-closed；
