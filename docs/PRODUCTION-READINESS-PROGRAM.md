...
> **† 价值判断非红线**：gate-design 标"强烈建议而非硬性 BLOCK"。owner 可降回 Gate2-M2 完整项——此处提 P0 因成本已确认极低（计算逻辑已存在）且直击"完备预案"的 MTTD（平均发现时间）。**此项去留由 owner 拍。**
...
1. **P0 各项判据全绿**（B1/B2/B3/N1/N2 五硬项 + M2† 心跳切片，M2† 去留由 owner 拍）——每项附其 is-True/is-False 判定 + tamper 实证 + 证据链接（见 §6 追踪表），非无归属勾选。
...
| Gate1 | P0-M2† | worker 心跳上 health（价值判断，owner 拍） | ⬜ | — | — |
