...
> 红线承接:信任色锁五槽/诚实地板句/诚实纪律不纳入「可交易项」(ADR-0039 主权层口径);「折叠保 DOM」为 MAP32 A2 验收红线(`MAP32-DESTINATION-ACCEPTANCE-record.md:28`),同样不纳入。凡差距项可能误碰红线的逐条标注。
> 关联:map #46 · #47(PR #50 行话审计)· #48(PR #49 可信内核盘点)· #51(PR #54 词表)· #56(PR #59 触发-浮现面)。
> 互审修正(2026-08-06,gpt-5.6):红线出处分离(折叠保 DOM=MAP32 A2)/后端事件证据链更正/人日区间重算/虚拟滚动移出交易项/依赖口径(4+3)/分布表闭合/字体与标识措辞精确化/MAP32 E1·E3 挂账声明。
...
- 副标变体(半套):静态 title `frontend/index.html:7` 作「工程智能体运行底座」(**脱「二所」**),与侧栏/登录门的「二所工程智能体运行底座」不一致;运行时 title 由**生产主应用**唯一写手 `utils/titleBadge.js:7`(base=`"FLAi-OS"`)接管、`router/index.js:54` 合成(ui-lab 验收台另有独立 title 写入,`ui-lab/main.js:23`,DEV ONLY)——首屏静态 title 与路由后 title 是两套字面。

### 2.2 视觉身份:标识四套并立,无独立品牌色族,字体纯系统栈

- **标识**:
  - 侧栏/hero 同源:`App.vue:16`、`GuidePage.vue:9` → `components/artwork/FlaiBloom.vue:13` → `assets/flai-bloom.png`(clay 螺旋花 PNG)。
...
- **字体**:纯系统栈(`App.vue:369-372` serif/sans/mono),`index.html` 零字体链路,全仓零 @font-face——**未引入随包字体,暂无随包字体授权负担**;代价=跨平台渲染不一致风险:字体栈含 Iowan Old Style/Songti SC 等 macOS 系字体,目标机为内网低配 Windows(`MOTION-SYSTEM.md:35-36` 自述)——**目标 Windows 字体回落与跨平台字形差异未实测**。
...
- **遗留英文面**(#47 P2-2/P2-6 现状):
  ...
  - raw event token:`WorkLog.vue:90` 展开态直出 `e.event_type`(`:69-71` 注释自述有意设计)——英文枚举=event_type token 面;事件正文为后端中文串夹英文字段名/插值(`repos.py:834-836` review_approved「人工批准放行(reviewer={reviewer}),任务转 completed」、`:846-848` review_rejected 同款;`:803` 为任务行 error_message 同款口径),正文直出 `WorkLog.vue:92`;前端事件标签翻译表 `format.js:48-68`(**18 键**,未知类型 fallback 原 token 裸显)。
...
| B4 | 字体跨平台一致性未实测(无随包字体授权负担✓)(`App.vue:369-372`) | 维持系统栈;或内嵌 OFL 开源字体(思源宋/黑类)——须过 bundle 预算与零依赖纪律,bundle 影响未核 | 1-2 人日+裁决 |
...
| B8 | 事件面英文残留:event_type token 直出(`WorkLog.vue:90`)+后端事件正文夹 `reviewer=` 等英文字段(`repos.py:834-848`→`WorkLog.vue:92`) | 后端事件正文结构化(reviewer= 迁 payload 字段,信息零删除)或前端 WorkLog 翻译层;event_type 展开态直出为有意设计,留 owner 裁是否一并收口 | 3 处后端串+测试锚,1-2 人日 |
...
- 前端依赖=运行时 4 项+开发 3 项(`frontend/package.json`),**无 vue-i18n**;`grep i18n frontend/` 0 命中,无自研 `t()`。唯一 locale 先例:Element Plus 中文化注入(`main.js:3-4` zhCn)——EP 侧换语言包只需换这一个 import,应用文案层不存在。
- 后端无本地化层:面向用户的 `HTTPException(detail="中文")` 22 处、runner 事件 message 中文 ~10 处、`raise 中文` 76 处(大部分内部);`str(exc)` 拼接面见 #47 P1-2。
- **天然切分点(已存在的键结构)**:`format.js:48-68` `EVENT_TYPE_LABEL`(后端 token→前端译,18 键)等约 15 张枚举映射表;`router/index.js:11-26` meta.title 8 条——键空间结构已在代码里隐含,只是没有词典层。
...
- 分布(闭合):views 1894 / components 1152 / utils 834 / ui-lab 215(验收 fixture,非生产)/ stores 134 / api 150 / router 36 / root+composables+effects 276(App.vue 250 等)。
- 分离注释后,**实际需抽取的用户可见文案为粗略上界 ≈1700 行**(模板 ~550 + 属性 ~68 + JS 字面量 ~1121)。注意口径:此为「含引号中文串的行数」未经逐项归类(混入日志/fixture/内部串),行数≠文案实例数,仅作量级上界;精确清单需按「模板文本节点/属性实例/JS 字符串实例」逐文件归类,本盘点未做。
...
- **SSOT 抽取容易度:高**——format.js 约 15 张映射表纯函数+node 直测,~0.5-1 人日,且可成为键体系种子。
...
- 后端事件正文含人名/枚举插值(repos.py:834-848),本地化须连同 §2.3 B8 一起处理。
...
- **切片与可加总区间**(人日粗估,口径=串数×机械替换均速+锚迁移开销,含自测、不含翻译审校;锚迁移 3-6 粗拆为壳层相关 2-4 + 后端相关 1-2):
  - 切片 0「词典框架」(两案之一):**1-2 人日**。
  - 切片 1「SSOT 先行」:format.js 约 15 表+router meta+utils 键化 → **2-4 人日**。
  - 切片 2「壳层文案外搬」:40 个 .vue 模板+属性+script toast → **8-15 人日**(GuidePage/TaskDetail/App/StatusCenter 四巨头占一半)。
  - 切片 3「锚迁移随行」(不可省):e2e 25 文件 2683 行中文锚+node 32 文件 1074 行+backend/tests 87 文件 4835 行 → 全量 **3-6 人日**,策略=锚改断言 t() 键或双语文案表。
  - 切片 4「后端本地化」(detail 22+runner message ~10,可选):**2-3 人日**。
  - **MVP(切片 0+1+2+壳层锚 2-4,不翻后端)≈ 13-25 人日;全量(MVP+后端切片 4+后端锚 1-2)≈ 16-30 人日。**
...
- **预警项:GuidePage 路由闭包 JS gzip 206,057 B / CSS 34,148 B——已达路由预算 91.5% / 85.4%,余量仅 19,223 B(≈18.8 KiB)/ 6,812 B(≈6.7 KiB),下一个中等功能即撞门。**
...
- GuidePage 对 messages 全量 v-for(`GuidePage.vue:41`),中段折叠=display:none 保 DOM(`:3393`)——**折叠保 DOM 是 #25/MAP32 A2 红线**(MAP32 验收记录:28),长会话 DOM 随轮数线性无界增长;后端会话接口全量返回消息(无分页,`runtime/conversation.py:320`)。
...
- 已知观察项:MAP32 E2(首段过长豁免)/E5(滚动还原恒差 14px)仍开;另有**交互稳定性挂账** E1(peek 焦点断档)/E3(展开后滚段首未实现)——属 a11y/交互档,#45 retro 小批候选,**本档(资源/运行稳定性)不覆盖,单列说明**。
...
| P3 | 长会话 DOM 无界+流式全量重算(§4.3) | delta 节流(rAF 合批)/流式期纯文本 done 后富渲染/历史消息分页加载(后端分页参数+前端分段渲染,不动已渲 DOM);**虚拟滚动直接破坏折叠保 DOM 语义(MAP32 A2 红线),当前红线下禁止采用,不列为交易项——改红线须另起治理裁决** | 节流 1-2 人日;分页 2-3 人日 |
...
2. **GuidePage 预算撞门处置**(P1):拆异步子 chunk vs 提预算——候选排序建议拆(预算纪律是既有防漂移机制),留 owner 裁。
3. **多语言做不做/何时做**:MVP ≈13-25 人日(词典层+SSOT+壳层外搬+壳层锚迁移)vs 全量 ≈16-30 人日 vs 不做(内网单语种场景可论证);**若做,框架两案须先裁依赖红线**(vue-i18n=新依赖,零依赖纪律外;自研词典层=零依赖但缺 ICU)。
...
7. **优先级备料(候选排序,不替裁)**:低成本高一致性收益项(B1/B6/B7/B9/B11+P4/P5 ≈ 4-6 人日)可作批 2 首票候选;P2 定档与多语言裁决为批 2 方向性前置。

## 六、证据边界与未核项

- 本报告以静态盘点+构建实测为主,**未起体验栈实测**;Windows 字体回落、动效文档↔生产逐条对照、role=alert 各 detail 来源、样张 @dsCard 仓外消费方为未核项(§二标注)。
- 中文行统计为「含引号中文串的行数」上界(未逐项归类,混入日志/fixture/内部串),行数≠文案实例数;人日为粗估口径,各切片组成已在 §3.5 列明可加总。
- 行号锚自 `6459031`;三路子代理盘点输出未入库,关键结论已并入本报告并逐条复核源码;互审(86gs 中转 gpt-5.6,tokens 177,250)12 条 findings 全部处置(修正/精确化),处置留痕见 PR #60 评论。
