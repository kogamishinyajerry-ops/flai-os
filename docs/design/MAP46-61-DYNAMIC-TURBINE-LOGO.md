...
> 状态：**设计稿，未施工**。本文件 + `map46-61-dynamic-turbine/` 预览稿 + `docs/reviews/map46-61-turbine-shots/`
> 截图为全部产出；零产品代码改动。定稿=owner 逐项交互选择题（§六），定稿后另立施工票。
...
- 已有动态先例：`FlaiBloom.vue:46-54` generating 态整枚旋转 3.6s/圈 linear infinite，
  仅绑 `m.streaming`（`GuidePage.vue:77`）；reduced-motion 降级块 `:56-62` 已在。
...
| ii（难度分档） | 同 i | 同 i | 流式在途 **且** 本卡召集成员任务 ≥2 在工作态（多人会战=「难」的确定性代理）；单任务流式落中速 3.6s（现行观感） | 四档（含中速），最贴 owner 原话「难任务→高速」，代理=成员任务计数（确定性） |
...
- MOTION-SYSTEM 六条：动画只 transform/opacity；reduced-motion=静态（每稿自带媒体查询块 + 预览模拟开关，
  真实 `reduced_motion="reduce"` 上下文实测：点高速 currentTime 恒 0）；换档 ramp 用瞬时 rAF（<0.5s 自清理，
  硬约束⑤瞬时例外同型），常驻循环零新增；零第三方依赖（CSS/WAAPI 原生）。✓
...
- **Q1 形态**：甲（原花提速，最保守）/ **乙（SVG 涡轮重绘，推荐候选）** / 丙（乙+高速尾迹，辨识度最强）；
  丙可降级为「乙先行、尾迹后补」两阶段。
...
- 预览稿全部 `file://` 可开、零依赖、零网络；截图用本机 Playwright chromium 148。
