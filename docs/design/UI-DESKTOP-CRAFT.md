...
- **Q5 详情页开发者语言收折**：折叠态=人话扫读面（G1/G2 头行/工具聚合行/
  ...
  「核验/签发/授权链」信任核心面不动。
  **折叠态升格的诚实义务**（Codex R0 P1 + R1-P1/P2 收口）：工具聚合行折叠
  常显 ⇒ mock 徽数据必须随工具**终结**事件到账预载（run 行只在工具结束/
  失败时落库，started 时拉必空），不得等展开；数据面=`/tool_runs/summary`
  的 `by_tool` 有界投影（tool_id+计数纯元数据，绝不为折叠态搬整条执行
  轨迹）；**loaded≠已核**——逐工具对账，「有 tool 事件、无对应 by_tool 行」
  （运行中/未执行/敏感遮蔽无法归因）与未拉/加载中/拉取失败同律亮 amber
  「真实性未核」，未知绝不呈现成「非 mock」（mock 如实标注红线在主扫读面
  的落地形态）。
