...
def _grounding_status(reply: str, hits: list[dict[str, Any]]) -> tuple[bool, list[str]]:
    ...
    （Codex R2-P1 假绿闸）。返回 (grounded, invented_keys)：

    - grounded=True ⟺ 答复至少引用一个命中 hits 的有效键 **且无**编造键（引用了检索命中之外的键）。
      任一编造键即翻 False——半真半造的引用比全无引用更危险，不放行冒充 grounded。
    - invented_keys = 答复引用了但不在 hits 内的键（去重排序，供警示横幅列示）。

    有效键集 = {(source.strip, chunk_id.strip)}（同名文件 chunk 会重复，故用 source·chunk **复合**
    键定位，单 chunk 不唯一——与 prompt/出处表口径一致）。失败方向保守：解析不到有效引用（无引用/
    分隔符偏离/编造）一律 grounded=False → 调用方标 amber 未核（宁标未核，绝不把未核答复冒充 grounded）。
    诚实边界：正则启发式非语义理解，模型换引用格式会被判未核（fail-safe 方向），V0.2 可升级。
    """
    valid_keys = {(str(h["source"]).strip(), str(h["chunk_id"]).strip()) for h in hits}
    cited: set[tuple[str, str]] = set()
    for m in _CITATION_KEY_RE.finditer(reply):
        cited.add((m.group(1).strip(), m.group(2).strip()))
    invented = sorted(
        f"[{s} · {c}]" for (s, c) in cited if (s, c) not in valid_keys
    )
    has_valid = any((s, c) in valid_keys for (s, c) in cited)
    grounded = has_valid and len(invented) == 0
