...
Agent 中推荐一个，并按其 `input_schema.json` 预填一份任务草案，交用户确认后
由用户提交；确认时会话附件随草案带入创建任务页，一次上传直达任务。
...
- **做**：接住自然语言需求与会话附件（≤5 个/条；文本类直读、`.xlsx` 预览
  ...
  specialist Agent；生成经确定性校验的预填任务草案；确认草案时把会话附件
  带到创建任务页（是否随任务提交由人决定）。
- **不做**：不解析 docx/pdf 等其余类型（只列文件名，V0.3 规划）；不产出任何
  工程产物；**不创建、不签发下游任务**（预填草案交人在创建任务页确认提交——
  人是唯一签发者）；不推荐已下线（disabled）的 Agent 或它自己。
