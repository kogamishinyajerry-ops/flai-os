...
def run_benchmark(*, db_dir: Path, n_tasks: int, n_submitters: int,
    ...
    # 3) 单串行排干；每 run_once 后查新完成任务、记完成单调+墙钟时刻。
    #    A5-P2-3：drain **每轮**采最老 queued 龄期留 max——串行 drain 的龄期峰值出现在排干后段
    #    （最后一条 queued 等得最久），只采「提交后-worker 启动前」的 initial 会漏掉峰值。
    #    诚实边界：本 per-round get_queue_depth 观测与既有 per-round completed 扫描同属 drain 期
    #    观测开销、含入 drain_wall_s——benchmark 是粗粒度规模天花板量测（非精确吞吐，两次重跑仅
    #    做数量级 sanity），此观测开销与既有扫描同量级、不改数量级结论。
    completion_mono: dict[str, float] = {}
    seen: set[str] = set()
    ...
    while True:
        ...
        c = conn_factory()
        try:
            rows = c.execute("SELECT id FROM tasks WHERE status = 'completed'").fetchall()
        finally:
            c.close()
        for row in rows:
            rid = row[0]
            if rid not in seen:
                seen.add(rid)
                completion_mono[rid] = now
    drain_wall_s = time.monotonic() - drain_start

    # 4) 收集逐任务记录 + fail-closed 终态核对（finished_at 由 DB 权威落 completed 时刻）。
    c = conn_factory()
    try:
    ...
    for tid, enq in sorted(enqueue_mono.items()):
        comp = completion_mono.get(tid)
        status = final_status[tid]
        latency_ms = (comp - enq) * 1000.0 if comp is not None else None
        records.append({
