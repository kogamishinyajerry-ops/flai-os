...
# 龄期采样固定预算（Codex R3-P2）：drain 期 get_queue_depth 采样总次数上限——每次采样是
# O(剩余队列) 聚合，固定次数令总观测开销 O(预算×N)=O(N) 线性（逐轮采样会退化 O(N²)，大 N 时
# drain_wall_s 量的是 instrumentation 而非 JobRunner）。64 对 200~10k 任务给出足够的峰值分辨率。
_AGE_SAMPLE_BUDGET = 64
...
def run_benchmark(*, db_dir: Path, n_tasks: int, n_submitters: int,
    ...
    # 3) 单串行排干；龄期采样**限固定次数预算**（Codex R2-P2 → R3-P2 verbatim 收口）。
    #    A5-P2-3：drain 期采最老 queued 龄期留 max——串行 drain 的龄期峰值出现在排干后段（最后
    #    一条 queued 等得最久），只采「提交后-worker 启动前」的 initial 会漏掉峰值。
    #    ★Codex R2-P2 完成扫描：旧实现每 run_once 后 SELECT 全部 completed 行再 Python 去重=O(N²)
    #    总观测开销。完成时刻改 DB 权威 finished_at 落库、drain 结束后**末次一扫**（O(N) 总量）。
    #    ★Codex R3-P2 龄期采样：每轮 get_queue_depth 自身是 O(剩余队列) 聚合，逐轮采=O(N²) 观测
    #    开销计入 drain_wall_s（大 N 时量的仍是 instrumentation 非 JobRunner）。改**固定采样预算**
    #    （≤_AGE_SAMPLE_BUDGET 次，等距轮次采样）→ 总观测开销 O(预算×N)=O(N) 线性。诚实边界：
    #    龄期峰值分辨率降为 ceil(N/预算) 个任务的 drain 跨度（峰值在采样格点间可短暂更高不被记录）；
    #    benchmark 本就是数量级粗测（两遍重跑仅 sanity），max_queued_age_s 是观测参考位非 gate。
    max_queued_age_s = initial_depth.get("oldest_queued_age_s") or 0.0
    sample_every = max(1, math.ceil(n_tasks / _AGE_SAMPLE_BUDGET))
    round_idx = 0
    drain_start = time.monotonic()
    while True:
        if round_idx % sample_every == 0:
            c = conn_factory()
            try:
